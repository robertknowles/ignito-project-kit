  ...
  const getNextBestAction = () => {
    if (!clientStatus?.hasOnboardingId) {
      return {
        ...
        color: 'text-neutral-700 bg-neutral-50 border border-neutral-200',
      };
    }
      ...
      <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-3">
            ...
            <div>
              <div className="text-lg font-semibold text-neutral-900">{client.name}</div>
        ...
        <div className="space-y-6 mt-4">
          ...
          <div className={`rounded-lg p-4 ${nextAction.color}`}>
            <div className="flex items-start gap-3">
              ...
              <div>
                <h3 className="font-medium text-sm text-neutral-500">Next Best Action</h3>
                <p className="text-sm font-semibold mt-0.5">{nextAction.title}</p>
                <p className="text-xs mt-1 opacity-75">{nextAction.description}</p>
              </div>
            </div>
          </div>
            ...
            {profile ? (
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="bg-neutral-50 rounded-lg p-3">
                  ...
                  <div className="text-sm font-semibold text-neutral-900">
                    {profile.timelineYears || 20} years
                  </div>
                </div>
            ...
            <div className="space-y-3 max-h-[200px] overflow-y-auto">
              {communicationLog.length > 0 ? (
                communicationLog.map((entry) => (
                  <div key={entry.id} className="bg-neutral-50 rounded-lg p-3">
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-xs font-medium text-neutral-700">{entry.author}</span>
                ...
                <div className="text-center py-4">
                  <p className="text-sm text-neutral-500">No notes yet. Add your first note above.</p>
                </div>
              )}
            </div>
          </div>
