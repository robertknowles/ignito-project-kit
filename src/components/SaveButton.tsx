...
export const SaveButton: React.FC<SaveButtonProps> = ({ iconOnly = false }) => {
  ...
  return (
    <button
      onClick={saveScenario}
      disabled={isLoading || !hasUnsavedChanges}
      className="flex items-center gap-2 h-8 bg-[#3b82f6] bg-opacity-60 hover:bg-opacity-80 text-white px-3 rounded-md text-sm transition-all disabled:opacity-30 disabled:cursor-not-allowed"
    >
      {isLoading ? (
