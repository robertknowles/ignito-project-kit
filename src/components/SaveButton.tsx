...
export const SaveButton: React.FC<SaveButtonProps> = ({ iconOnly = false }) => {
  ...
  return (
    <button
      onClick={saveScenario}
      disabled={isLoading || !hasUnsavedChanges}
      className="flex items-center gap-1.5 bg-[#3B82F6] text-white px-4 py-2 rounded-lg font-medium text-[13px] hover:bg-[#2563EB] transition-colors shadow-md disabled:opacity-50 disabled:cursor-not-allowed"
    >
      {isLoading ? (
        <Clock size={15} className="animate-spin" />
      ) : (
