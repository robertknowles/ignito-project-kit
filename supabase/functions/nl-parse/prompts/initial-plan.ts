...
- Optional: address, boughtYear, rentPerWeek, interestRate, loanType ("IO" or "PI"), allowEquityRelease (boolean), saleYear (number)
...
- allowEquityRelease: set to true (default) if the property's equity can be used to fund future purchases. Set to false if the BA says "don't refinance this one", "exclude from refi", "no equity release", "PPOR — don't touch", etc.
- saleYear: set to the year the BA plans to sell the property (e.g. "sell the unit in 2030", "offload in 5 years" → current year + 5). Omit if no sale is planned.
