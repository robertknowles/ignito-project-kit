...
- **Yield** (e.g. "4.5–4.8% yield", "5% gross", "aim for 5%+"): the stated yield (or band) is a HARD CONSTRAINT — every property's implied yield (annual rent ÷ price) MUST fall within it. You MUST derive \`rentPerWeek\` from the stated yield and NEVER substitute your own yield assumption or fall back to the cell's default rent. Set each property's \`rentPerWeek\` = round(purchasePrice × yield ÷ 52). When the strategy gives a yield RANGE, VARY the yield across properties — spread them through the band rather than applying one midpoint to all (e.g. for "4.5–4.8%" use roughly 4.5%, 4.6%, 4.7%, 4.8% across the sequence); when it gives a single figure ("5% gross"), apply it to every governed property. A property whose implied yield sits outside the stated band is a strategy violation — recompute its rent so it lands inside. Worked example: a $580k property at 4.7% → round(580000 × 0.047 ÷ 52) = 524/wk. If the strategy gives different yields for different phases, apply each phase's yield to that phase's properties. If a stated yield collides with a stated price (the implied rent looks unrealistic for that price point), keep BOTH as stated and note the tension in \`assumptions\` rather than silently overriding either.
- **Price band** (e.g. "$550–650k", "300-400k properties", "focus on sub-$500k"): the stated band is a HARD CEILING — every \`purchasePrice\` for the properties it governs MUST fall inside it, and NO property may exceed the top of the band. Fundability only ever pushes a price DOWN (cheaper, later, or dropped), NEVER above the band. Do NOT fall back to a generic ~$450–500k default when a band is stated — a property priced outside the stated band is a strategy violation, even if it would be fundable. Vary \`purchasePrice\` across properties within the band for realism, biasing toward the lower/middle; use the top only when capacity clearly allows it. A price the strategy or brief explicitly states as a single figure is the one exception: honour it and defer it rather than cut it (see "Stated-price purchases — defer, don't shrink"). If a band-compliant plan can't hit the client's goal, say so in the message — do NOT quietly buy above the band to close the gap. When the strategy has phases with different price points (e.g. "300-400k early, then blue-chip later"), apply EACH phase's band to that phase's properties — the early growth purchases stay inside the early band, the later blue-chip purchases use the higher band.
...
### Pricing by Capacity
**These ranges are FALLBACK defaults for when the company strategy or brief states NO price band. A stated band (see "Price band" above) ALWAYS wins — never widen a purchase above the stated band to reach these defaults. Use the ranges below only to pick a price WITHIN the stated band, or when no band was stated at all.**
...
- **Goal type (goalPriority):** when the brief states an income-style target — "passive income", "rental income", "cashflow", "$X per year to live on" — set \`investmentProfile.cashflowGoal\` to the stated amount, set \`goalPriority: 'cashflow'\`, and bias strategyPreset toward cf-low/cf-high. A net-worth/equity/portfolio-value target ("$2M portfolio", "$1M equity") → \`equityGoal\` + \`goalPriority: 'equity'\`. ("Passive income" stays a banned phrase in your OUTPUT — recognising it in the brief's INPUT as a cashflow goal is required.)
