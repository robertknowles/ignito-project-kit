    ...
    properties: {
      ...
      investmentProfile: {
        ...
        properties: {
          ...
          goalPriority: { type: 'string' as const, enum: ['equity', 'cashflow'], description: 'Which goal the client leads with. "cashflow" when the brief states a passive-income, rental-income, cashflow, or "$X/year income" target; "equity" for a net-worth, equity, or portfolio-value target. Omit only when no goal is stated at all.' },
    ...
    properties: {
      profileUpdates: {
        ...
        properties: {
          ...
          existingPropertyEquity: { type: 'number' as const },
          targetPassiveIncome: { type: 'number' as const },
          goalPriority: { type: 'string' as const, enum: ['equity', 'cashflow'], description: 'Which goal the client leads with. "cashflow" for a passive-income/rental-income/"$X per year" target; "equity" for a net-worth or portfolio-value target. Only when the BA states or changes the goal.' },
